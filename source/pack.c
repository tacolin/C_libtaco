...
static unsigned char _getBits(unsigned char* bytes, int ofs, int bit_num)
{
...
static void _putBits(unsigned char* bytes, int ofs, int bit_num, unsigned char value)
{
...
unsigned int bits_get(void* bytes, int* ofs, int bit_num)
{
    ...
    {
        ret = _getBits((unsigned char*)bytes, *ofs, bit_num);
        *ofs += bit_num;
    }
    else if ((bit_offset(*ofs) + bit_num) < 40)
    {
        bits -= (8 - bit_offset(*ofs));
        tmp   = _getBits(start, bit_offset(*ofs), (8 - bit_offset(*ofs))) << bits;
        ret  |= tmp;
        ...
        tmp  = _getBits(stop, 0, bit_offset(*ofs+bit_num)) << bits;
        ret |= tmp;
...
void bits_put(void* bytes, int* ofs, int bit_num, unsigned int value)
{
    ...
    if ((bit_offset(*ofs) + bit_num) <= 8)
    {
        _putBits((unsigned char*)bytes, *ofs, bit_num, (unsigned char)value);
    }
    else
    {
        bits -= (8 - bit_offset(*ofs));
        _putBits(start, bit_offset(*ofs), (8 - bit_offset(*ofs)), value >> bits);
        ...
        bits -= bit_offset(*ofs+bit_num);
        _putBits(stop, 0, bit_offset(*ofs+bit_num), value & (0xFF >> (8 - bit_offset(*ofs+bit_num))));
